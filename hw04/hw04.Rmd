...
mean_sd <- function(x){
  c( qnorm(0.025,mean(x),sd(x)), qnorm(0.975,mean(x),sd(x)) )
}
...
histColumn <- function(col,d){
    quantiles <- trans_params[,col]
    data <- d[order(d[,col]),c(col,target_bin)]
    x <- data[,col]
    y <- cumsum(data[,target_bin])/sum(data[,target_bin])
    y_inv <- (sum(data[,target_bin])-cumsum(data[,target_bin]))/sum(data[,target_bin])
    plot(y~x, main=col)
    points(x,y_inv,col="red")
    data
}


sapply(inputs_num,function(x){histColumn(x,training_trans)})

col <- "KIDSDRIV"
    quantiles <- trans_params[,col]
    x <- training_trans[,col]
    hist(c(x[ x < quantiles[1] ],x[ x > quantiles[2] ]), main=col)
