...
binResponse <- function(col,d){
#  par(mfrow=c(2,1))
    data <- d[order(d[,col]),c(col,target_bin)]
    x <- data[,col]
    y <- cumsum(data[,target_bin])/sum(data[,target_bin])
    y_inverse <- (sum(data[,target_bin])-cumsum(data[,target_bin]))/sum(data[,target_bin])
    plot(y~x, main=paste0("TARGET_FLAG ~ ",col," Response"))
    points(x,y_inverse,col="red") # adding inverse so it gives a better visual appreciation of what's going on
#    x <- log(data[,col])
#    plot(y~x, main=paste0("TARGET_FLAG ~ log(",col,") Response"))
#    points(x,y_inverse,col="red") # adding inverse so it gives a better visual appreciation of what's going on
}


sapply(inputs_num,function(x){binResponse(x,training_trans)})
...
Let's see what the individual plots look like:

```{r}

###############

lmResponse <- function(col,d){
    data <- d[order(d[,col]),c(col,target_lm)]
    x <- data[,col]
    y <- data[,target_lm]
    plot(y~x, main=paste0("TARGET_LM ~ ",col," Response"))
}


nnn<-sapply(inputs_num,function(x){lmResponse(x,training_trans[training_trans$TARGET_FLAG==1,])})

```

From the charts above, I fail to see a linear response to any of the targets. Let's apply a log transformation on the target and see if it changes:
...
lmResponse <- function(col,d){
    ...
    plot(y~x, main=paste0("log(TARGET_LM) ~ ",col," Response"))
}


sapply(inputs_num,function(x){lmResponse(x,training_trans[training_trans$TARGET_FLAG==1,])})
...
Let's split the dataset into training and testing so that we end up with two datasets:

* for logistic regression (training_flag, testing_flag)
* for linear regression (training_num, testing_num)
