...
library(MASS)
valid_data <- training_transformed[!training_transformed$INVALID&complete.cases(training_transformed),colnames(training_transformed)[!colnames(training_transformed) %in% c("TEAM_PITCHING_H","TEAM_BATTING_H","INVALID")]]
...
#  y_power <- 1
  ...
  res <- data.frame(try=paste0(label,"^",x_power),
                    ...
                    x_power=x_power,
                    y_power=y_power,
#             intercept=s$coefficients[,"Estimate"][1],
#             intercept_p=s$coefficients[,"Pr(>|t|)"][1],
