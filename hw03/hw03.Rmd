...
cap <- function(x){
    quantiles <- c( mean(x)-3*sd(x), mean(x)+3*sd(x) )
...
```{r}
valid_data <- transformed
predictors <- inputs[!inputs %in% c("indus","chas","lstat","rm")]
model <- glm(I(target)~.,data=valid_data[,c(predictors,target)],family = binomial)
